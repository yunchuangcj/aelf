...
namespace AElf.Execution
{
    public class ConcurrencyExecutingService : IConcurrencyExecutingService
    {
        ...
        public async Task<List<TransactionTrace>> ExecuteAsync(Dictionary<Hash,List<ITransaction>> transactionsWithChainId, IGrouper grouper)
        {
            if (!_isInit)
            {
                InitActorSystem();
            ...
            _requestor = _actorSystem.ActorOf(Requestor.Props(_router));
            var executeService = new ParallelTransactionExecutingService(_requestor, grouper);
            return await executeService.ExecuteAsync(transactionsWithChainId);
        }
